...
bool FlashMemory::xWriteAndCheck(uint32_t address, const void *data, uint32_t size)
{
	vSectorEarse(address);
	vWriteData(address, (const uint8_t *) data, size);
	vReadData(address, (uint8_t *) mBuffer, size);
